...
This rather large regular expression *just happened* to match a perfectly innocent piece of text in my `Comprehensive Linux Backups with etckeeper & backupninja <{filename}comprehensive-linux-backups-with-etckeeper-backupninja.adoc>`_ article - specifically, this directory listing matched the ``|drwxr))`` bit at the end of that regex:
