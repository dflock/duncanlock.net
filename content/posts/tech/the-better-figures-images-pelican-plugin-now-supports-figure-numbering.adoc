...
So, the link:++{static}/posts/tech/better-figures-and-images-plugin-for-pelican.rst++[Better Figures & Images plugin] now supports automatic figure numbering. To enable this for all posts, just add this to your config file:
...
While we're on that subject, this plugin link:++{static}/posts/tech/how-i-upgraded-this-website-to-pelican-33.rst++[does work with Pelican 3.3], but that's not upstream yet either - the `figure_numbers` branch includes those fixes too.
