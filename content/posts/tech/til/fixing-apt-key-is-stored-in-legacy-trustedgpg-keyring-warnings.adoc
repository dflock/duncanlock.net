...
I chose to put these into `/usr/share/keyrings/`, which seems to be the "default" location.
    ...
    /usr/share/keyrings is for keys installed by the packet manager and are part of a package. (in fact, everything under /usrshould be handled by the packet manager.)
    /etc/apt/keyrings is for other keys added to the system. (for example, the key of a manually configured external repo.)
